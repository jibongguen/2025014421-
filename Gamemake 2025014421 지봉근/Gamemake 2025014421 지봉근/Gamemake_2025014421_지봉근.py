...
class wolf:
    def __init__(self,canvas,image,id):
        self.id='w'+str(id) #늑대 고유 식별 번호#
        self.canvas=canvas 
        self.image=image
        self.wolf = self.canvas.create_image(960,random.randint(10,470), image = self.image,tags=self.id) #늑대 소환 위치, 이미지, 태그아이디#
        self.wolfmove=0

    def update(self):
        self.canvas.move(self.wolf,-4,0) #각 늑대 움직임#

    def wolfPos(self):
        return self.canvas.coords(self.wolf) #현재 위치 반환#

    def wolfId(self):
        return self.wolf #자기 정보 반환#
...
class pig:
    def __init__(self,canvas,image,id):
        self.id='p'+str(id) #늑대 고유 식별 번호#
        self.canvas=canvas 
        self.image=image
        self.pig = self.canvas.create_image(960,random.randint(10,470), image = self.image,tags=self.id) #늑대 소환 위치, 이미지, 태그아이디#
        self.pigrush = 0
    ...
    def update(self):
        self.canvas.move(self.pig,self.pigrush,0) #각 늑대 움직임#
        self.pig_now_time=time.time()
    ...
    def pigPos(self):
        return self.canvas.coords(self.pig) #현재 위치 반환#

    def pigId(self):
        return self.pig #자기 정보 반환#
        
class elephant:
    def __init__(self,canvas,image,id):
        self.id='el'+str(id) #늑대 고유 식별 번호#
        self.canvas=canvas 
        self.image=image
        self.elephant = self.canvas.create_image(960,random.randint(10,470), image = self.image,tags=self.id) #늑대 소환 위치, 이미지, 태그아이디#
        self.el_life=5

    def update(self):
        self.canvas.move(self.elephant,-2,0) #각 늑대 움직임#

    def elephantPos(self):
        return self.canvas.coords(self.elephant) #현재 위치 반환#

    def elephantId(self):
        return self.elepahnt #자기 정보 반환#
...
class GamePlay:
        ...
        while True:
            ...
            try:
                ...
                if gameover == 1 and realgameover == 0 and question == 0:
                    for key in self.keys: #사냥꾼 움직임 및 총#
                        if key == 83:                 
                            gameover = 0
    ...
    def wolfmanage(self): #늑대 소환, 삭제 관리#
        ...
        hitbox = 30
        for f in self.firelist[:]:  # firelist 복사본으로 순회
            f_pos = self.canvas.coords(f)
            if not f_pos:  # 이미 삭제된 경우
                self.firelist.remove(f)
                continue

            for w in self.wolflist[:]:  # wolflist 복사본으로 순회
                w_pos = w.wolfPos()
                if not w_pos:  # 이미 삭제된 경우
                    self.wolflist.remove(w)
                    continue

        # 충돌 판정
                if (w_pos[0]-hitbox < f_pos[0] < w_pos[0]+hitbox) and (w_pos[1]-hitbox < f_pos[1] < w_pos[1]+hitbox):
                    self.canvas.delete(w.wolf)   # 늑대 삭제
                    self.wolflist.remove(w)
                    self.canvas.delete(f)        # 총알 삭제
                    self.firelist.remove(f)
                    self.score+=100
    ...
    def eaglemanage(self): #늑대 소환, 삭제 관리#
        self.eaglesummon=time.time()
    ...
    def pigmanage(self): #늑대 소환, 삭제 관리#
        self.pigsummon = time.time()
        ...
        # 충돌 판정
                if (p_pos[0]-hitbox < f_pos[0] < p_pos[0]+hitbox) and (p_pos[1]-hitbox < f_pos[1] < p_pos[1]+hitbox):
                    self.canvas.delete(p.pig)   # 돼지 삭제
                    self.piglist.remove(p)
                    self.canvas.delete(f)        # 총알 삭제
                    self.firelist.remove(f)
                    self.score+=150
    ...
    def elephantmanage(self): #늑대 소환, 삭제 관리#
        self.elephantsummon = time.time()
        ...
        for f in self.firelist[:]:  # firelist 복사본으로 순회
            ...
            for el in self.elephantlist[:]:  # wolflist 복사본으로 순회
                el_pos = el.elephantPos()
                if not el_pos:  # 이미 삭제된 경우
        ...
        # 충돌 판정
                if (el_pos[0]-hitbox < f_pos[0] < el_pos[0]+hitbox) and (el_pos[1]-hitbox < f_pos[1] < el_pos[1]+hitbox):
                    ...
                    el.el_life-=1
                    if el.el_life <=0:
                        self.canvas.delete(el.elephant)   # 늑대 삭제
                        self.elephantlist.remove(el)
    ...
    def gameover (self):
        ...
        for e in self.eaglelist[:]:
            self.canvas.delete(e.eagle)   # 독수리 삭제
            self.eaglelist.remove(e)
            print("eagle")
        ...
        for el in self.elephantlist[:]:
            self.canvas.delete(el.elephant)   # 돼지 삭제
            self.elephantlist.remove(el)
            print("elephant")

        for st in self.stonelist[:]:
            self.canvas.delete(st.stone)   # 돼지 삭제
            self.stonelist.remove(st)
            print("stone")

        for f in self.firelist[:]:
            self.canvas.delete(f)   # 돼지 삭제
            self.firelist.remove(f)
            print("fire")
